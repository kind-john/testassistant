...
public class CameraViewPhotos extends TestItemBase {
    public static final int ID = 89;
    private static final String TITLE = "Camera View Photos";
    private static final String TAG = "CameraViewPhotos";

    public CameraViewPhotos() {
        super();
    ...
    public CameraViewPhotos(Context context) {
        super(context);
        String className = this.getClass().getName();
        setClassName(className);
        setID(ID);
    ...
    public boolean doExecute(UseCaseManager.ExecuteCallback executeCallback, boolean finish) {
        LogUtils.d(TAG, "CameraViewPhotos doExecute");
        new Thread() {
            public void run() {
                try{
                    Point point = mUseCaseManager.getTouchPosConfig(PointConstants.CAMERA_VIEW_PHOTO_POINT);
                    if(point != null){
    ...
    public void showPropertyDialog(Context context, final boolean isNeedUpdateXml) {
        ...
        builder.setTitle("CameraViewPhotos settings")
                .setView(v)
                .setMessage("for test")
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if(delay >= 0 && times > 0){
                            ...
                            setTimes(times);
                            if(isNeedUpdateXml){
                                mUseCaseManager.updateTestItemOfAllUseCaseXml(CameraViewPhotos.this);
                            }
                        }
                    }
                })

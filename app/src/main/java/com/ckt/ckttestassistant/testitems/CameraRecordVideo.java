...
public class CameraRecordVideo extends TestItemBase {
    public static final int ID = 62;
    private static final String TITLE = "Camera Record Video";
    private static final String TAG = "CameraRecordVideo";

    public CameraRecordVideo() {
        super();
    ...
    public CameraRecordVideo(Context context) {
        super(context);
        String className = this.getClass().getName();
        setClassName(className);
        setID(ID);
    ...
    public boolean doExecute(UseCaseManager.ExecuteCallback executeCallback, boolean finish) {
        LogUtils.d(TAG, "CameraRecordVideo doExecute");
        new Thread() {
            public void run() {
                try{
                    Point point = mUseCaseManager.getTouchPosConfig(PointConstants.CAMERA_RECORDVIDEO_POINT);
                    if(point != null){
    ...
    public void showPropertyDialog(Context context, final boolean isNeedUpdateXml) {
        ...
        builder.setTitle("CameraRecordVideo settings")
                .setView(v)
                .setMessage("for test")
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if(delay >= 0 && times > 0){
                            ...
                            setTimes(times);
                            if(isNeedUpdateXml){
                                mUseCaseManager.updateTestItemOfAllUseCaseXml(CameraRecordVideo.this);
                            }
                        }
                    }
                })

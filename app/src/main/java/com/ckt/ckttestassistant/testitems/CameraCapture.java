...
public class CameraCapture extends TestItemBase {
    public static final int ID = 61;
    private static final String TITLE = "Camera Capture";
    ...
    public boolean doExecute(UseCaseManager.ExecuteCallback executeCallback, boolean finish) {
        LogUtils.d(TAG, "CameraCapture doExecute");
        new Thread() {
            public void run() {
                try{
                    Point point = mUseCaseManager.getTouchPosConfig(PointConstants.CAMERA_CAPTURE_POINT);
                    if(point != null){
    ...
    public void showPropertyDialog(Context context, final boolean isNeedUpdateXml) {
        ...
        builder.setTitle("CameraCapture settings")
                .setView(v)
                .setMessage("for test")
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if(delay >= 0 && times > 0){
                            ...
                            setTimes(times);
                            if(isNeedUpdateXml){
                                mUseCaseManager.updateTestItemOfAllUseCaseXml(CameraCapture.this);
                            }
                        }
                    }
                })

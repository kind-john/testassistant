...
public class CameraEnterPhotoMode extends TestItemBase {
    public static final int ID = 85;
    private static final String TITLE = "Camera Enter Photo Mode";
    private static final String TAG = "CameraEnterPhotoMode";

    public CameraEnterPhotoMode() {
        super();
    ...
    public CameraEnterPhotoMode(Context context) {
        super(context);
        String className = this.getClass().getName();
        setClassName(className);
        setID(ID);
    ...
    public boolean doExecute(UseCaseManager.ExecuteCallback executeCallback, boolean finish) {
        LogUtils.d(TAG, "CameraEnterPhotoMode doExecute");
        new Thread() {
            public void run() {
                try{
                    Point point = mUseCaseManager.getTouchPosConfig(PointConstants.CAMERA_PHOTOMODE_POINT);
                    if(point != null){
    ...
    public void showPropertyDialog(Context context, final boolean isNeedUpdateXml) {
        ...
        builder.setTitle("CameraEnterPhotoMode settings")
                .setView(v)
                .setMessage("for test")
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if(delay >= 0 && times > 0){
                            ...
                            setTimes(times);
                            if(isNeedUpdateXml){
                                mUseCaseManager.updateTestItemOfAllUseCaseXml(CameraEnterPhotoMode.this);
                            }
                        }
                    }
                })

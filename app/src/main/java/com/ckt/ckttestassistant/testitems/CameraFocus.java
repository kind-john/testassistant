...
public class CameraFocus extends TestItemBase {
    public static final int ID = 65;
    private static final String TITLE = "Camera Focus";
    ...
    public boolean doExecute(UseCaseManager.ExecuteCallback executeCallback, boolean finish) {
        LogUtils.d(TAG, "CameraFocus doExecute");
        new Thread() {
            public void run() {
                try{
                    HashMap<String, Point> points = mUseCaseManager.getTouchPosConfig();
                    Point point = points.get(MyConstants.CAMERA_FOCUS_POINT);
                    if(point != null){
    ...
    public void showPropertyDialog(Context context, final boolean isNeedUpdateXml) {
        ...
        builder.setTitle("CameraFocus settings")
                .setView(v)
                .setMessage("for test")
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if(delay >= 0 && times > 0){
                            ...
                            setTimes(times);
                            if(isNeedUpdateXml){
                                mUseCaseManager.updateTestItemOfAllUseCaseXml(CameraFocus.this);
                            }
                        }
                    }
                })
